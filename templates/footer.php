
<!-- #footer -->
<div id="footer" class="container-fluid">
	<div class="row">
			<div class="col-xs-12 col-md-10 col-md-offset-1 col-sm-12">

		<div class="col-md-4 col-xs-6 col-sm-4">
			<h4 class="upper line">Encontranos en</h4>
			<a href="ubicacion.php">
				<figure>
					<iframe width="300" height="150" class="iframe-footer" frameborder="0" scrolling="no" marginheight="0" marginwidth="0" src="https://maps.google.com/maps?q=-38.532221,-58.769871&amp;num=1&amp;ie=UTF8&amp;t=h&amp;ll=-38.532187,-58.770504&amp;spn=0.030214,0.051413&amp;z=14&amp;output=embed"></iframe>
				</figure>
			</a>
		</div>
		<div class="col-md-4 col-xs-6 col-sm-4">
			<h4 class="upper line">Buscanos en</h4>
				<iframe class="iframe-footer" src="//www.facebook.com/plugins/likebox.php?href=https%3A%2F%2Fwww.facebook.com%2Fpages%2FDon-Francisco-Maquinarias%2F160607427288244&amp;width=300&amp;height=200&amp;show_faces=true&amp;colorscheme=light&amp;stream=false&amp;show_border=false&amp;header=false&amp;appId=185172994932129" scrolling="no" frameborder="0" class="embed-responsive-item" allowTransparency="true"></iframe>
			</div>
			<div class="col-md-4 col-xs-12 col-sm-4">
					<h4 class="upper line">Don Francisco</h4>
						<div class="textwidget">
							<p>La  empresa DON FRANCISCO comienza con la actividad en Necochea en la comercialización  de maquinarias agrícolas y ganaderas en el año 2000.</p>
							<p> Desde ese momento y hasta  la fecha se destaca como una de las principales en rubro en Necochea y la zona.</p>
							<p class="text-right"><strong>Don Francisco Maquinarias</strong></p>
						</div>
				</div>
			</div>

		<!-- end - #footer -->

		<!-- #footer-contact-info -->
				<div class="col-xs-12 col-md-12 col-sm-12 contact-info">

					<ul>
						<li class="space">
							<div>
								<span class="glyphicon glyphicon-home space" aria-hidden="true"></span>Ruta 228 Km. 2, Necochea. Bs As (ARG)
							</div>
						</li>
						<li class="space">
							<div>
							<span class="glyphicon glyphicon-phone-alt space" aria-hidden="true"></span>(02262) 421369
						</div>
						</li>
						<li class="space">
							<div>
							<span class="glyphicon glyphicon-phone space" aria-hidden="true"></span>(02262) 15567543
						</div>
						</li>
						<li class="space">
							<div>
							<span class="glyphicon glyphicon-envelope space" aria-hidden="true"></span>
								<a class="contact" href="mailto:carinacools@donfranciscomaquinarias.com">carina@donfranciscomaquinarias.com</a>
							</div>
						</li>
						<li class="space facebook">
							<div>
								<i class="fa fa-facebook space" aria-hidden="true"></i>
								<a class="contact" href="https://www.facebook.com/pages/Don-Francisco-Maquinarias/160607427288244?fref=ts" target="_blank">Facebook</a>
							</div>
						</li>
						</ul>
					</div>
				</div>
			<!-- end - #footer-contact-info -->

			<!-- #footer-bottom-->
				<div class="row footer-bottom">
					<div class="col-md-offset-2 col-md-6 col-sm-8 col-xs-8">
							<ul>
								<li><a href="index.php">Inicio</a></li>
								<li><a href="nosotros.php">Nosotros</a></li>
								<li><a href="ubicacion.php">Ubicación</a></li>
								<li><a href="search.php">Nuevos y Usados</a></li>
								<li><a href="contacto.php">Contacto</a></li>
							</ul>
					</div>
						<div class="col-md-4 col-sm-4 col-xs-4">copyright © 2013 <a href="http://bpsistemas.com/" class="space" target="_blank">BPsistemas</a>
						</div>
					</div>

				</div>
			</div>

			</div>

				<!-- end - #footer-bottom -->
